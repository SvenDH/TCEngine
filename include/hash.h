...
static inline sid_t tc_hash_string(const char* s)
{
    return s ? tc_hash64(s, (uint32_t)strlen(s), 0) : 0;
}

static inline sid_t tc_hash_string_len(const char* s, uint32_t len)
{
    return s ? tc_hash64(s, len, 0) : 0;
}

static inline sid_t tc_hash_string_tolower(const char* s)
{
...
#define hash_foreach(_m, _k, _v) \
for (uint32_t __i = 0; __i < (_m)->cap; __i++) \
	if((_k) = (_m)->keys[__i], (_v) = (_m)->vals[__i], (_k) != 0) \

//TODO: Consider fixing code duplication
