...
#define CAS(_x, _y, _z) \
atomic_compare_exchange_strong((atomic_t*)(_x), (size_t*)&(_y), (size_t)(_z), MEMORY_ACQ_REL)
...
static INLINE
uint32_t load_32_relaxed(const atomic32_t* object)
{
    return object->_nonatomic;
}

static INLINE
void store_32_relaxed(atomic32_t* object, uint32_t value)
{
    object->_nonatomic = value;
}

static INLINE
uint32_t exchange_32_relaxed(atomic32_t* object, uint32_t desired)
{
...
static INLINE
intptr_t compare_exchange_weak_32_relaxed(atomic32_t* object, uint32_t* expected, uint32_t desired)
{
    uint32_t e = *expected;
...
static INLINE
void store_64_relaxed(atomic64_t* object, uint64_t value)
{
#if CPU_X64
    object->_nonatomic = value;
#else
...
static INLINE
uint64_t exchange_64_relaxed(atomic64_t* object, uint64_t desired)
{
#if CPU_X64
...
static INLINE
void store_32_relaxed(atomic32_t* object, uint32_t desired)
{
...
static INLINE
intptr_t compare_exchange_weak_64_relaxed(atomic64_t* object, uint64_t* expected, uint64_t desired)
{
    uint64_t e = *expected;
    uint64_t previous;
    intptr_t matched;
...
#endif
#endif
static INLINE uint32_t load_32_relaxed(const atomic32_t* object)
{
    return object->_nonatomic;
}

static INLINE void store_32_relaxed(atomic32_t* object, uint32_t desired)
{
    object->_nonatomic = desired;
}
...
#if PTR_SIZE == 4
INLINE
size_t atomic_load(const atomic_t* object, memory_order_t order)
{
    size_t result = load_32_relaxed((atomic32_t*)object);
...
INLINE
void atomic_store(atomic_t* object, size_t desired, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    store_32_relaxed((atomic32_t*)object, desired);
}

INLINE
size_t atomic_exchange(atomic_t* object, size_t desired, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    size_t result = exchange_32_relaxed((atomic32_t*)object, desired);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
INLINE
size_t atomic_fetch_add(atomic_t* object, ptrdiff_t operand, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    size_t result = fetch_add_32_relaxed((atomic32_t*)object, operand);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
INLINE
size_t atomic_fetch_and(atomic_t* object, size_t operand, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    size_t result = fetch_and_32_relaxed((atomic32_t*)object, operand);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
INLINE
size_t atomic_fetch_or(atomic_t* object, size_t operand, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    size_t result = fetch_or_32_relaxed((atomic32_t*)object, operand);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
INLINE
size_t atomic_compare_exchange(atomic_t* object, size_t expected, size_t desired, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    size_t result = compare_exchange_32_relaxed((atomic32_t*)object, expected, desired);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
INLINE
bool atomic_compare_exchange_weak(atomic_t* object, size_t* expected, size_t desired, memory_order_t success, memory_order_t failure)
{
    if ((success == MEMORY_RELEASE || success == MEMORY_ACQ_REL) ||
        (failure == MEMORY_RELEASE || failure == MEMORY_ACQ_REL))
        thread_fence_release();
    intptr_t result = compare_exchange_weak_32_relaxed((atomic32_t*)object, expected, desired);
...
INLINE
bool atomic_compare_exchange_strong(atomic_t* object, size_t* expected, size_t desired, memory_order_t order)
{
...
size_t atomic_load(const atomic_t* object, memory_order_t order)
{
    size_t result = load_64_relaxed((atomic64_t*)object);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
void atomic_store(atomic_t* object, size_t desired, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    store_64_relaxed((atomic64_t*)object, desired);
}
...
size_t atomic_exchange(atomic_t* object, size_t desired, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    size_t result = exchange_64_relaxed((atomic64_t*)object, desired);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
size_t atomic_fetch_add(atomic_t* object, ptrdiff_t operand, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    size_t result = fetch_add_64_relaxed((atomic64_t*)object, operand);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
size_t atomic_fetch_and(atomic_t* object, size_t operand, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    size_t result = fetch_and_64_relaxed((atomic64_t*)object, operand);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
size_t atomic_fetch_or(atomic_t* object, size_t operand, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    size_t result = fetch_or_64_relaxed((atomic64_t*)object, operand);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
size_t atomic_compare_exchange(atomic_t* object, size_t expected, size_t desired, memory_order_t order)
{
    if (order == MEMORY_RELEASE || order == MEMORY_ACQ_REL)
        thread_fence_release();
    size_t result = compare_exchange_64_relaxed((atomic64_t*)object, expected, desired);
    if (order == MEMORY_ACQUIRE || order == MEMORY_ACQ_REL)
        thread_fence_acquire();
    return result;
}
...
bool atomic_compare_exchange_weak(atomic_t* object, size_t* expected, size_t desired, memory_order_t success, memory_order_t failure)
{
    if ((success == MEMORY_RELEASE || success == MEMORY_ACQ_REL) ||
        (failure == MEMORY_RELEASE || failure == MEMORY_ACQ_REL))
        thread_fence_release();
    intptr_t result = compare_exchange_weak_64_relaxed((atomic64_t*)object, expected, desired);
    if (result && (success == MEMORY_ACQUIRE || success == MEMORY_ACQ_REL))
        thread_fence_acquire();
    else if (failure == MEMORY_ACQUIRE || failure == MEMORY_ACQ_REL)
        thread_fence_acquire();
