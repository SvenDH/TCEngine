...
inline uint32_t align(uint32_t val, uint32_t alignment) {
	TC_ASSERT(is_power_of_2(alignment), "[Math]: Alignment (%i) must be power of 2", alignment);
...
inline void color_to_vec(vec4 d, Color c) {
...
inline uint32_t crc32(const unsigned char* str, unsigned int len) {
